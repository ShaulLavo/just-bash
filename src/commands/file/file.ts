...
// Extension-based type detection for text files (file-type doesn't handle text)
...
// Map file-type ext to description (comprehensive list based on file-type package)
const EXT_TO_DESCRIPTION: Record<string, string> = {
	// Images
	jpg: 'JPEG image data',
...
function generateDescription(ext: string, mime: string): string {
	...
	if (EXT_TO_DESCRIPTION[ext]) {
		return EXT_TO_DESCRIPTION[ext]
	}
