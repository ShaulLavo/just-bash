...
describe('awk field handling', () => {
	...
	describe('field modification', () => {
		...
		it('should modify last field', async () => {
			const env = new Bash()
			const result = await env.exec(`echo "a b c" | awk '{ $NF = "C"; print }'`)
			expect(result.stdout).toBe('a b C\n')
			expect(result.exitCode).toBe(0)
		})
	})
	...
	describe('NF (Number of Fields)', () => {
		...
		it('should update when field is added', async () => {
			const env = new Bash()
			const result = await env.exec(`echo "a b" | awk '{ $4 = "d"; print NF }'`)
	...
	describe('special field cases', () => {
		...
		it('should handle trailing whitespace', async () => {
			const env = new Bash()
			const result = await env.exec(`echo "a b c   " | awk '{ print NF, $NF }'`)
		...
		it('should handle multiple consecutive delimiters', async () => {
			const env = new Bash()
			const result = await env.exec(`echo "a     b     c" | awk '{ print NF }'`)
			expect(result.stdout).toBe('3\n')
			expect(result.exitCode).toBe(0)
		})
	})
