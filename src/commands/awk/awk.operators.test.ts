...
describe('awk operators', () => {
	...
	describe('assignment operators', () => {
		it('should handle = assignment', async () => {
			const env = new Bash()
			const result = await env.exec(`echo "" | awk 'BEGIN { x = 10; print x }'`)
	...
	describe('operator precedence', () => {
		it('should handle multiplication before addition', async () => {
			const env = new Bash()
			const result = await env.exec(`echo "" | awk 'BEGIN { print 2 + 3 * 4 }'`)
		...
		it('should handle exponent before multiplication', async () => {
			const env = new Bash()
			const result = await env.exec(`echo "" | awk 'BEGIN { print 2 * 3 ^ 2 }'`)
