...
describe('xan join', () => {
	...
	it('performs right join with --right', async () => {
		...
		expect(result.exitCode).toBe(0)
		expect(result.stdout).toBe('id,name,user_id,score\n1,alice,1,100\n,,2,85\n')
	})
	...
	it('handles one-to-many joins', async () => {
		...
		const result = await bash.exec('xan join id /users.csv user_id /orders.csv')
		expect(result.exitCode).toBe(0)
		expect(result.stdout).toBe(
	...
	it('errors on missing key column', async () => {
		...
		const result = await bash.exec('xan join nonexistent /a.csv user_id /b.csv')
		expect(result.exitCode).toBe(1)
